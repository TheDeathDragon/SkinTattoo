...
## v11e 记录（2026-07-04 凌晨，游戏内迭代）

游戏内验证 v11d 后连续发现并修复四层问题：

1. **面部 mtrl 键检测漏判**：原版/pores 脸部 mtrl `shader_keys` 为 0 个——CategorySkinType 靠
   shpk 头部默认值路由，而 **skin.shpk 的默认值就是 Face (0xF5673524)**。检测改为"无显式键 =
   默认 Face"。（身体 mod 都显式写 ValBody。）
2. **唇缘过渡带发光**：原版脸法线 alpha 是 0/255 双平台 + 抗锯齿过渡带（1~254 全值扫过），
   任何被分配的发光行都会被过渡带扫中 -> 唇缘光轮廓。修复：RowIndex 合成基线 alpha 量化到
   {0,255}（cb7 肤色槽位查找仍拿到合法平台值）。
3. **采样期插值残线**：量化后 GPU 双线性过滤仍在 0/255 交界重新产生中间值（一个采样宽度的细线）。
   修复：注入代码的法线取样改为纹素中心对齐（resinfo -> floor+0.5 -> 回除，等效最近邻）。
4. **发光常量泄漏**：所有族共享的 pass[4] PS 原生读取 `cb0[3].x`（= g_EmissiveColor.R）。
   mtrl 中写入非零合并色会经原生通道泄漏（唇部区域被合并色染亮）。修复：native 族 mtrl 的
   g_EmissiveColor 常量写黑（颜色完全由 CT 提供）。
5. **脖子接缝（部分）**：v11d 后接缝 = 脸原生 Face 族 vs 身体强制 Emissive 族的跨族差异。
   修复：把 per-PS 参数化注入扩展到 **Body/BodyJJM 族**（各 32 PS，离线 96/96 验证），身体
   mtrl 也保持原生键（Eve 的 ValBody 键原样保留），Emissive 强制仅剩显式 Emissive 键的 mtrl。
6. **投影贴花跨组网格错乱**：合成器投影用全局 `currentMesh`（跟随 UI 选中组），处理其它组时
   投到外族 UV -> 0 像素（"切换图层贴花消失"）；且 UI 异步网格加载与合成竞态（同一次预览
   diffuse 正常 / normal 为 0）。修复：`GetProjectionMeshForGroup` 按组提取+缓存（不碰全局），
   `AsyncLocal<MeshData>` 合成作用域贯穿全部下游合成（full / inplace / export 三条路径）；
   加 `[CompositeNorm] RowIndex NxN: k px painted` 自诊断计数。

SchemaVersion v11d -> v11e。

### 遗留问题（下一步）

- **唇缘仍有极细描边**（量化+纹素对齐后仍在）。候选解释：
  (a) 量化把唇部遮罩边缘硬化，原生唇色渲染的硬边视觉上像描边——鉴别方法：看线条颜色，
      发光色 = 仍是发光泄漏；肤色/深色 = 量化副作用。
  (b) 若是 (b)，改进设计：**放弃量化**（保留原版软唇边，原生唇色渲染无损），注入代码里改为
      "k*17 网格门控"：纹素对齐取样后，alpha 距离最近 k*17 (k=1..7) 超过 ±ε 则视为行 0。
      过渡带字节值恰等于 k*17 的像素约 1/255 × 带宽，可忽略。
- **脖子接缝残留**：用户判断是 mod 材质差异（身体 oily 材质有油性高光，脸 pores 材质没有）——
  验证方法：关闭插件/清除重定向后对比，接缝仍在则与插件无关。
- pass[4] 读 cb0[3].x 的具体用途未逆向（已用常量置黑绕过）。
- Emissive 族 pulse 锚点只匹配 r1（32 个里 8 个带动画）——native 族注入固定 r1 不受影响，
  Emissive 族修复留待需要时做。
