...
**v2 方案**（payload 改动极小）：
1. CT 行 V 从常数改为 `mad rA.y, v1.y, l(0.0625), l(0.09375)` —— 左眼取 rows 2/3、
   右眼取 rows 4/5（行距 2/32=0.0625）；
2. 发光色搬入 CT col2（halfs 8..10，预平方存），payload 用 `ctColor² × mask` **替换**
   原生 rE（movc/mov 覆盖），mtrl g_EmissiveColor 置黑（iris 无泄漏 pass，安全）——
   由此双眼可异色、异强度、异动画参数；
3. MtrlFileWriter 烘两组行；DecalLayer/TargetGroup 增加右眼参数字段（注意
   SavedLayer/LayerSnapshot DTO 同步）；UI 增左右眼控件；
4. 预览侧 hook 只有单色通道（cb0[3] 双眼共享）——预览要么保持单色近似，要么把
   iris_ct 部署进预览（预览统一化）后走 CT 双眼路径。
