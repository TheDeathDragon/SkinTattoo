...
public class DxRenderer : IDisposable
{
    ...
    // Reusable scratch buffer to avoid LOH thrash during sustained 3D editor drags
    private byte[]? bgraScratch;
    private readonly object bgraScratchLock = new();
    ...
    private void CompileShaders()
    {
        ...
        string hlslSource;
        using (var stream = assembly.GetManifestResourceStream("SkinTattoo.Shaders.Model.hlsl"))
        {
            if (stream == null) throw new Exception("Shader resource 'SkinTattoo.Shaders.Model.hlsl' not found");
    ...
    public ShaderResourceView CreateTextureFromRgba(byte[] rgbaData, int texWidth, int texHeight, out Texture2D texture)
    {
        ...
        lock (bgraScratchLock)
        {
            ...
            {
                bgraScratch[i] = rgbaData[i + 2];
                bgraScratch[i + 1] = rgbaData[i + 1];
                bgraScratch[i + 2] = rgbaData[i];
                bgraScratch[i + 3] = rgbaData[i + 3];
            }
    ...
    /// <summary>Create an R8G8B8A8 texture for repeated sub-region updates (no swizzle needed).</summary>
    ...
    /// <summary>Update a sub-rect of a texture from a full-sized RGBA source buffer.</summary>
