...
docs/                                 # Technical documentation
  ...
  cbuffer-atlas.md                    # 跨 shpk cbuffer 图谱: 名字哈希算法/RDEF 成员布局/m_LoopTime 时间源/iris_ct 注入设计与实现
```
...
- **Iris animation on export: FIXED (2026-07-04, iris_ct).** `Services/IrisShpkPatcher.cs` (schema iris2) REPLACES the native `g_EmissiveColor^2 * mask.r` term in all 64 iris lighting/composite PSes with CT-driven glow: per-eye row pair selected by vertex color v1.y (left = rows 2/3, right = rows 4/5 -- the vanilla heterochromia discriminator), color in CT col2 pre-squared, anim (speed/amp/mode/colorB/dual) in cols 3/4/6, time = `g_PbrParameterCommon.m_LoopTime` at cb2[0].x. The mtrl ShaderPackageName is rewritten to `iris_ct.shpk` (rename-deploy, always-on shared asset in the pmp). Per-eye split UI: DecalLayer.EyeSplitEnabled/EmissiveColorRight/EmissiveIntensityRight (iris groups only). Preview is UNIFIED with export: when animation or eye-split is configured, the preview temp mod deploys the same iris_ct.shpk + CT mtrl (`irisCtMaterials` marker) and live edits hot-swap the CT via `TryWriteIrisCtDirect` / the inplace worker (`ReplaceColorTableRaw` 8x32); the CBuffer hook is skipped for iris-CT materials (a uniform-CT write would stomp the per-eye rows). Static single-color iris with no anim/split stays on the legacy hook path. CBuffer/slot ground truth in `docs/cbuffer-atlas.md`.
- **Lip rim glow on face emissive: FIXED, in-game confirmed (2026-07-04).** Root-cause chain across versions: forced-Emissive routing (fixed by v11d native routing) -> anti-aliased alpha sweep lighting CT rows (v11e quantization, which itself mutated vanilla skin-type IDs -> thin hard outline) -> v11f in-shader k*17 grid gate -> residual stray pixels were AA-band bytes RANDOMLY landing on exact k*17 values passing the gate (~1/18 per grid value), fixed by nudging non-plateau exact-grid baseline alphas +1 in the composite plus real-baseline allocator occupancy scan. The final "sparkle" was diagnosed as bloom optics of a high-intensity (x10) glow on vanilla lip specular glints -- verified via black-CT A/B test, negligible at sane intensities, not a leak.
...
- **v11d/v11e (2026-07)**: native-routing rework. CT emissive is injected into ALL Face/Body/BodyJJM lighting PSes (per-PS parameterized slots, texel-snapped normal fetch) and skin mtrls keep their native CategorySkinType keys; g_EmissiveColor constant is zeroed for native mtrls (shared pass[4] reads cb0[3].x natively and leaks it). See `docs/shader-emissive-vfx-exploration.md` "v11e 记录" for the full engineering log incl. the projector per-group mesh race fix.
- **v11f (2026-07-04)**: the lip-rim thin outline root cause was v11e's baseline alpha quantization itself -- normal.alpha doubles as the engine's 32KB skin-type LUT index (g_ShaderTypeParameter), so snapping the texture to 0/255 plateaus mutated vanilla lip shading (and flattened body-mod skin IDs like Eve's, likely worsening the neck seam). v11f removes the texture quantization entirely and instead gates in-shader: the injection zeroes any sampled alpha that is not an exact k*17 multiple (k <= 14) before the CT row lookup, so vanilla anti-aliased bands neither glow nor lose their skin IDs. The gate is per-material switchable via CT col3 half15 (gateFlag=1 in per-layer CTs, 0 in NormalEmissive ramp CTs which need the continuous sweep). RowPairAllocator no longer assigns pair 15 (alpha 255 = vanilla lip mask, always gated).
