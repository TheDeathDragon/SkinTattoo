...
public partial class MainWindow
{
    ...
    {
        // Header: [添加] + name + model/mtrl info
    ...
    private void ExportAllDiagnostics()
    {
        ...
        var sb = new System.Text.StringBuilder();
        sb.AppendLine($"# SkinTatoo 诊断 dump @ {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
        ...
        DebugServer.AppendLog($"[MainWindow] Exported diagnostics ({sb.Length} chars) to clipboard");
    }
    ...
    private void AddTargetGroupFromMtrl(ResourceNodeDto mtrlNode, ResourceNodeDto? parentMdl)
    {
        ...
        foreach (var diag in resolution.Diagnostics)
            DebugServer.AppendLog($"[SkinMeshResolver] {diag}");
